  ...
  return {
    ...
    configureServer(server) {
      ...
      server?.httpServer?.once('listening', () => {
        ...
        // 启动Electron进程
        let electronProcess = spawn(require('electron'), ['dist/main-process.js', IP])

        // 监听主进程代码的更改
        fs.watchFile('src/main-process.ts', () => {
          // 杀死当前的Electron进程
          electronProcess.kill()
          // 重新编译主进程代码并重新启动Electron进程
          initElectron()
          electronProcess = spawn(require('electron'), ['dist/main-process.js', IP])
        })
